...
namespace Testing
{
    ...
    {
        public class RotorsNotMVVM
        {
            private List<RotorNotMVVM> _list = new List<RotorNotMVVM>();

            //исправить
            private RotorNotMVVM _keyboard = HistoricData.Keyboard;

            public RotorNotMVVM Reflector { get; private set; }

            //исправить
            public RotorsNotMVVM()
            {
                this.Reflector = HistoricData.Reflectors.ReflectorB;
            }

            //+
            public void Add(RotorNotMVVM RotorNotMVVM, char head)
            {
                RotorNotMVVM rotor = RotorNotMVVM;

                rotor.SetHead(head);
                this.Add(rotor);
            }

            public void Add(RotorNotMVVM rotor)
            {
            ...
            //+
            public void SetReflector(RotorNotMVVM reflector)
            {
                RotorNotMVVM rotor = reflector;
